...
import AddNewSeriesSearchResult from './AddNewEventSearchResult';
import { useLookupSeries } from './useAddEvent';
import styles from './AddNewEvent.css';
      ...
      <PageContentBody>
        ...
        {!isFetching && !error && !!data.length ? (
          <div className={styles.searchResults}>
            {data.map((item) => {
              return (
                <AddNewSeriesSearchResult key={item.tvdbId} event={item} />
              );
            })}
          </div>
        ) : null}
        ...
        {!isFetching && !error && !data.length && term ? (
          <div className={styles.message}>
            ...
            <div>
              <Link to="https://wiki.servarr.com/fightarr/faq#why-cant-i-add-a-new-event-when-i-know-the-tvdb-id">
